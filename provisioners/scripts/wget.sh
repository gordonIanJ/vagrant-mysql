...
# Patch file name
printf 'Patch file name e.g. p27890946_56_Linux-x86-64.zip: '
...
#
$WGET  --secure-protocol=auto --save-cookies="$COOKIE_FILE" --keep-session-cookies  --http-user "$SSO_USERNAME" --ask-password  "https://updates.oracle.com/Orion/Services/download" -O /dev/null |tee $LOGFILE 
...
then
 ...
 $WGET  --load-cookies="$COOKIE_FILE" "https://updates.oracle.com/Orion/Services/download/$PATCH_FILE_NAME?aru=$ARU&patch_file=$PATCH_FILE_NAME" -O "$OUTPUT_DIR/$PATCH_FILE_NAME" -O "$OUTPUT_DIR/$PATCH_FILE_NAME" |tee "$LOGFILE"
